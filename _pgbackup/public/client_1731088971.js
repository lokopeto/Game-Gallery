document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        Object.keys(folderMap).forEach(folderName => {
            ...
            // Add images to the folder div, each wrapped in a downloadable link
            folderMap[folderName].forEach(imageSrc => {
                const imgElement = document.createElement("img");
                imgElement.src = imageSrc;
                imgElement.classList.add("image-item");

                // Wrap the image in a downloadable link
                const link = document.createElement("a");
                link.href = imageSrc;
                link.download = imageSrc.split('/').pop(); // Set download attribute to filename
